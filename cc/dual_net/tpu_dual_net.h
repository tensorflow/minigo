...
class TpuDualNet : public DualNet {
 public:
  ...
  // buffering: Number of allowed overlapped calls to Session::Run.
  //            Should be 2 to enable double buffering, which currently gets the
  //            best performance.
  // max_batch_size: maximum size of each batch passed to Session::Run.
  //                 Should be virtual_losses * parallel_games / buffering.
  TpuDualNet(const std::string& graph_path, const std::string& tpu_name,
             int buffering, int max_batch_size);
  ~TpuDualNet() override;
