...
arguments:
 ...
 - **tpu**: perform inference on a Cloud TPU. Your code must run on a Cloud
   ...
   Use by passed `--model=tpu:$BUFFER_COUNT:$TPU_NAME,$PATH`, where
   `$BUFFER_COUNT` is the number of inference requests to the TPU to overlap
   in parallel (try even values between 2 and 8), `$TPU_NAME` is the name of
   a Cloud TPU (e.g. `grpc://10.240.2.10:8470`) and `$PATH` is the path to a
