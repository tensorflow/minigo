...
passing Bazel `--define` arguments at compile time. The C++ binaries can choose
to use any of the compiled engines at runtime using the `--engine` command line
argument:
 ...
 - **tf**: peforms inference using the TensorFlow libraries built by
   ...
   disable the engine with `--define=tf=0`.
 ...
 - **trt**: uses NVIDIA TensorRT.
   ...
   Use by passing `--engine=trt` when you run your compiled binary.
 - **fake**: a simple fake that is only useful in so much as you can use it to
   ...
   Use by passing `--engine=fake` when you run your compiled binary.
...
Build `//cc:selfplay` with `--define=tpu=1` and run with `--engine=tpu`.
